<style>
/* Par défaut, afficher le carousel desktop et masquer le carousel mobile */
.carousel-desktop {
    display: block;
}
...
<div class="container">
    <div class="text-center mx-auto mb-2 mt-2 wow fadeInUp" data-wow-delay="0.1s" style="max-width: 600px;">
        ...
        <center><img src="../assets/img/section-img.png" alt="Section Image" style="max-width: 100%;"></center>  
    </div>
    ...
    @if ($doctors->count() <= 4)
        ...
        <div class="row mx-2">
            @foreach ($doctors as $doctor)
                <!-- Debug: Image path: {{ $doctor->image ? asset('storage/' . $doctor->image) : asset('assets/img/doctors/default.jpg') }} -->
                <div class="col-12 col-md-3 single-pf">
                    <div class="team-item position-relative rounded overflow-hidden wow fadeInUp" data-wow-delay="0.1s">
                        <div class="overflow-hidden">
                            <img class="img-fluid" 
                                 src="{{ $doctor->image ? asset('storage/' . $doctor->image) : asset('assets/img/doctors/default.jpg') }}" 
                                 alt="{{ $doctor->name }}">
                        </div>
                        <div class="team-text bg-light text-center p-4">
                            ...
                            <p class="text-secondary">
                                @if($doctor->schedule)
                                    @php
                                        $schedules = explode('|', $doctor->schedule, 2);
                                        $schedule1 = $schedules[0] ?? '-';
                                        $schedule2 = $schedules[1] ?? ($schedules[0] ? '-' : null);
                                    @endphp
                                    {{ $schedule1 }}
                                    @if($schedule2)
                                        <br>{{ $schedule2 }}
